...
export default async function DashboardPage() {
  ...
  // Fetch real data stats scoped by user_id
  ...
  // Get conversations that need takeover
  ...
  // Calculate stats (fallback to mockup indicators if new account, to match the Figma mockup)
  ...
  // Mock graph coordinates for SVG sparkline
    ...
    >
      ...
      <div className="dashboard-kpi-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "var(--space-4)" }}>
        <div style={{ background: "#FFFFFF", padding: "var(--space-4)", borderRadius: "var(--radius-xl)", border: "var(--border-default)" }}>
          ...
          <div style={{ fontSize: "1.8rem", fontWeight: "bold", margin: "8px 0" }}>{totalConvs.toLocaleString()}</div>
          <div style={{ fontSize: "var(--size-caption)", color: "var(--color-muted-foreground)" }}>
            All messages handled by your AI agent
          </div>
        </div>
        ...
        <div style={{ background: "#FFFFFF", padding: "var(--space-4)", borderRadius: "var(--radius-xl)", border: "var(--border-default)" }}>
          ...
          <div style={{ fontSize: "1.8rem", fontWeight: "bold", margin: "8px 0" }}>{totalLeads}</div>
          <div style={{ fontSize: "var(--size-caption)", color: "var(--color-muted-foreground)" }}>
            Qualified buyers identified from chats
          </div>
        </div>
        ...
        <div style={{ background: "#FFFFFF", padding: "var(--space-4)", borderRadius: "var(--radius-xl)", border: "var(--border-default)" }}>
          ...
          <div style={{ fontSize: "1.8rem", fontWeight: "bold", margin: "8px 0" }}>{convRate}%</div>
          <div style={{ fontSize: "var(--size-caption)", color: "var(--color-muted-foreground)" }}>
            Percentage of conversations that convert
          </div>
        </div>

        <div style={{ background: "#FFFFFF", padding: "var(--space-4)", borderRadius: "var(--radius-xl)", border: "var(--border-default)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", color: "var(--color-muted-foreground)" }}>
            <span style={{ fontSize: "var(--size-label)", fontWeight: "bold", textTransform: "uppercase" }}>AI Handled</span>
            <MessageSquare size={16} />
          </div>
          <div style={{ fontSize: "1.8rem", fontWeight: "bold", margin: "8px 0" }}>{aiHandled}</div>
          <div style={{ fontSize: "var(--size-caption)", color: "var(--color-muted-foreground)" }}>
      ...
      <div className="dashboard-charts-grid" style={{ display: "grid", gridTemplateColumns: "1.3fr 0.7fr", gap: "var(--space-5)" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "var(--space-4)" }}>
            <div>
              ...
              <p style={{ fontSize: "var(--size-caption)", color: "var(--color-muted-foreground)" }}>
                AI replies handled vs human takover
              </p>
            </div>
            <span style={{ fontSize: "var(--size-label)", color: "var(--color-primary-text)", fontWeight: "bold" }}>
        ...
        >
          <div>
            <h3 style={{ fontSize: "var(--size-h3)", fontWeight: "bold" }}>Takeover Alerts</h3>
            <p style={{ fontSize: "var(--size-caption)", color: "var(--color-muted-foreground)" }}>
              ...
              escalatedChats.map((chat) => (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: "var(--size-body-sm)", fontWeight: "bold", fontFamily: "var(--font-mono)" }}>
                      {chat.customerPhone}
                    </span>
                    <span style={{ fontSize: "var(--size-micro)", color: "var(--color-destructive)", fontWeight: "bold", textTransform: "uppercase" }}>
                      Score: {Math.round(chat.aiConfidenceScore * 100)}%
                    </span>
                  </div>
                  <div style={{ fontSize: "var(--size-caption)", color: "var(--color-muted-foreground)", textOverflow: "ellipsis", overflow: "hidden", whiteSpace: "nowrap" }}>
                    {/* Display latest customer text */}
                    {(chat.conversationHistory as any)?.[(chat.conversationHistory as any).length - 1]?.content || "Inquiry needs handoff"}
                  </div>
                  <Link
                    href="/conversations"
                    style={{
      ...
      <div style={{ background: "#FFFFFF", border: "var(--border-default)", borderRadius: "var(--radius-xl)", padding: "var(--space-5)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "var(--space-3)" }}>
          <div>
            <h3 style={{ fontSize: "var(--size-h3)", fontWeight: "bold" }}>Total Customer Questions</h3>
            <p style={{ fontSize: "var(--size-caption)", color: "var(--color-muted-foreground)" }}>Common questions your AI agent has handled</p>
          </div>
        </div>
        {dbConversationsCount === 0 ? (
          <div style={{ padding: "var(--space-4) 0", textAlign: "center", color: "var(--color-muted-foreground)", fontSize: "var(--size-caption)" }}>
            No questions yet. Questions will appear here once customers start chatting.
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "var(--border-default)", fontSize: "var(--size-body-sm)" }}>
              <span>"Is this available in stock?"</span>
              <span style={{ fontWeight: "bold", color: "var(--color-primary-text)" }}>12 times</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "var(--border-default)", fontSize: "var(--size-body-sm)" }}>
              <span>"What's the delivery fee?"</span>
              <span style={{ fontWeight: "bold", color: "var(--color-primary-text)" }}>8 times</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", fontSize: "var(--size-body-sm)" }}>
              <span>"Do you have this in size 12?"</span>
              <span style={{ fontWeight: "bold", color: "var(--color-primary-text)" }}>5 times</span>
            </div>
          </div>
        )}
      </div>
...
// Inline fallback checkmark icon to replace loading packages if they aren't generated yet
